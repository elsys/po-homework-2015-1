...
int main(){
	...
	{
		printf("%c: %d\n", alphabet[count_printer].letter_id, alphabet[count_printer].times);
	}
	return 0;
}
...
int sort_structure()
{
	...
	for(array_x = 0; array_x < 26 ; array_x++)
	{
		...
		{
			if(alphabet[array_y].times < alphabet[array_x].times)
			{
				swap_times = alphabet[array_x].times;
			...
			else if(alphabet[array_y].times == alphabet[array_x].times)
			{
